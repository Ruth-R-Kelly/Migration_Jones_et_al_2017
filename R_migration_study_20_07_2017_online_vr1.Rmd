...
authors: Ruth Kelly (kellyr44@tcd.ie), with some code for PCA analysis from Mahesh Poudyal 2017 
---
...
pcplot2 <- ggplot(pov_pca_scores, aes(PC1, PC2, shape=MigrantF, colour = MigrantF, fill = MigrantF)) +
 ...
 ylim(-4.2,6) + 
  ...
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey") + 
  theme(panel.grid = element_blank()) + 
  geom_point(data = centroids, aes(x = PC1, y = PC2), inherit.aes = FALSE) +
  geom_segment(data = centroids,aes(x = PC1[1] , y = PC2[1] + 0.1, xend = -0.5, yend = -3.2), inherit.aes = FALSE, colour = "#e34a33") + 
  annotate("text", x=-0.5, y=-3.5, label="Non-migrant", size=3,colour = "#e34a33") + 
  geom_segment(data = centroids,aes(x = PC1[2] , y = PC2[2] +0.05, xend = -1, yend = 3.2), inherit.aes = FALSE, colour = "#225ea8") + 
  annotate("text", x=-1.1, y=3.5, label="Migrant", size=3, colour = "#225ea8") + 
    theme(legend.position = c(0.7, 0.09), legend.text = element_text(size = 7)) +
  theme(axis.title = element_text(size = 11))  + 
...
### calculate convex hulls for  groups
find_hull <- function(x) x[chull(x$PC1, x$PC2), ]

hulls <- pov_pca_scores2 %>% group_by(ReasonF) %>% do(find_hull(.))
...
#### Colour palette to match map provided by Mahesh..  
# Land availability: #3ae22e
# Marriage: #e31a1c
# Followed family: #a6cee3
# Work: #1f78b4
# Other: #22eee8

## PCA scatter plot for all sites with convex hulls around scatter plot for each site (SI Figure S1-b)
pcplot3 <- ggplot(pov_pca_scores2, aes(PC1, PC2, shape=ReasonF, colour = ReasonF, fill = ReasonF)) +
  geom_point() + theme_bw() +
  geom_polygon(data = hulls, alpha = 0.1) +
  scale_shape_discrete(name=NULL, guide=guide_legend(reverse = F)) +
  scale_fill_manual(values = c("#3ae22e","#e31a1c",  "#a6cee3", "#1f78b4", "#22eee8"),name=NULL, guide=guide_legend(reverse = F)) +
    scale_colour_manual(values = c("#3ae22e", "#e31a1c",  "#a6cee3", "#1f78b4", "#22eee8"),name=NULL, guide=guide_legend(reverse = F) )+
  xlab("Wealth axis 1") +
  ylab(NULL) +
  annotate("text", x=-2.5, y=5.5, label="c)", size=7) +
   ylim(-4.2,6) + 
  xlim(-3,6) +  
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey") + 
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey") + 
  theme(panel.grid = element_blank()) + 
  geom_point(data = centroids, aes(x = PC1, y = PC2), inherit.aes = FALSE) +
  geom_segment(data = centroids,aes(x = PC1[1], y = PC2[1] + 0.05, xend = - 1, yend = 2.8), inherit.aes = FALSE, colour = "#3ae22e") + 
  annotate("text", x=-1.1, y = 3.2, label="Land", size=3, colour = "#3ae22e") +
    geom_segment(data = centroids,aes(x = PC1[2] - 0.05 , y = PC2[2] -0.05, xend = -1.1, yend = -1.6), inherit.aes = FALSE,colour = "#e31a1c") + 
  annotate("text", x=-1.3, y=-1.9, label="Marriage", size=3,colour = "#e31a1c") + 
  geom_segment(data = centroids,aes(x = PC1[3] , y = PC2[3] + 0.1, xend = -0.4, yend = 3.8), inherit.aes = FALSE, colour = "#a6cee3") + 
  annotate("text", x=-0.5, y=4.3, label="Family", size=3,colour = "#a6cee3" ) + 
  geom_segment(data = centroids,aes(x = PC1[4] -0.05 , y = PC2[4], xend = -1.7, yend = 1.1), inherit.aes = FALSE, colour = "#1f78b4") + 
  annotate("text", x=-2, y=1.4, label="Work", size=3, colour = "#1f78b4")  +
  geom_segment(data = centroids,aes(x = PC1[5], y = PC2[5] - 0.1, xend = 0.9, yend = -2.5), inherit.aes = FALSE, colour = "#22eee8") + 
  annotate("text", x=1, y=-2.8, label="Other", size=3, colour = "#22eee8") + 
  theme(legend.position = c(0.83, 0.15), legend.text = element_text(size = 7)) +
  theme(axis.title = element_text(size = 11))  + 
 guides(fill = guide_legend(keywidth = 0.5, keyheight = 0.5)) + 
  guides(colour = guide_legend(keywidth = 0.5, keyheight = 0.5)) + 
  guides(shape = guide_legend(keywidth = 0.5, keyheight = 0.5)) + 
...
# pdf("27_07_2017_pca_migrant_wealth_migrant_reason.pdf", width = 7.4, height = 3)
# grid.draw(cbind(ggplotGrob(pcplot1), ggplotGrob(pcplot2),ggplotGrob(pcplot3)))
# dev.off()
#
...
hulls <- pov_pca_scores %>% group_by(Site2) %>% do(find_hull(.))
...
pcplot4 <- ggplot(pov_pca_scores, aes(PC1, PC2, shape=Site2, colour = Site2, fill = Site2))+
  geom_point() + theme_bw() +
  geom_polygon(data = hulls, alpha = 0.1) +
  scale_colour_brewer(palette = "Dark2", name=NULL, guide=guide_legend(reverse = F)) +
  scale_fill_brewer(palette = "Dark2", name=NULL, guide=guide_legend(reverse = F)) +
  ...
  theme(panel.grid = element_blank()) + 
  geom_point(data = centroids, aes(x = PC1, y = PC2), inherit.aes = FALSE) +
  geom_segment(data = centroids,aes(x = PC1[1] - 0.05 , y = PC2[1] + 0.05, xend = -0.9, yend = 2.8), inherit.aes = FALSE, colour = "#1B9E77") + 
  annotate("text", x=-1.1, y=3.1, label="Ampahitra", size=3,colour = "#1B9E77") + 
  geom_segment(data = centroids,aes(x = PC1[2] , y = PC2[2]-0.05 , xend = -1.3, yend = -2.1), inherit.aes = FALSE, colour = "#D95F02") + 
  annotate("text", x=-1.4, y =-2.3,label="Sahavazina", size=3, colour = "#D95F02") +
  geom_segment(data = centroids,aes(x = PC1[3] , y = PC2[3], xend = -0.7, yend = -3.3), inherit.aes = FALSE,colour = "#7570B3") + 
  annotate("text", x=-0.7, y=-3.5, label="Mantadia", size=3,colour = "#7570B3") + 
  geom_segment(data = centroids,aes(x = PC1[4], y = PC2[4] + 0.05, xend = 0.5, yend = -2.7), inherit.aes = FALSE, colour = "#E7298A") + 
  annotate("text", x=0.6, y=-2.8, label="Zahamena", size=3, colour = "#E7298A")+  
  geom_segment(data = centroids,aes(x = PC1[5] + 0.05, y = PC2[5], xend = 3.9, yend = -0.126), inherit.aes = FALSE, colour = "#66A61E") + 
  annotate("text", x=4, y = 0.2, label="Amporoforo", size=3, colour = "#66A61E")  +
  theme(legend.position = c(0.83, 0.15), legend.text = element_text(size = 7)) +
  theme(axis.title = element_text(size = 11))  + 
 guides(fill = guide_legend(keywidth = 0.5, keyheight = 0.5)) + 
  guides(colour = guide_legend(keywidth = 0.5, keyheight = 0.5)) + 
...
# pdf("SuppA2_27_07_2017_pca_migrant_wealth_migrant_site.pdf", width = 7.4, height = 3)
...
plota <- ggplot(mig_only_200, aes(x = new_dist, fill = Site2, colour = Site2)) +             geom_density(aes(y = ..count..), alpha = 0.5, bw = 4) + 
       theme_bw() +
...
plotb <- ggplot(mig_only_200, aes(x = new_dist, fill = ReasonF, colour = ReasonF)) +     
    annotate("text", x=180, y=7.5, label="b)", size=8) +
       geom_density(aes(y = ..count..), alpha = 0.5, bw = 4) + 
       theme_bw() + 
  scale_fill_manual(values = c("#3ae22e", "#e31a1c", "#a6cee3", "#1f78b4", "#22eee8"),name=NULL, guide=guide_legend( reverse = F)) +  scale_colour_manual(values = c("#3ae22e", "#e31a1c","#a6cee3", "#1f78b4", "#22eee8"),name=NULL, guide=guide_legend( reverse = F)) +
       xlab("Distance travelled (km)") +
       ylab("Estimated migration") +
       xlim(0,200) + 
       ylim(0,8) + 
...
## output as PDF
# pdf("27_07_2017_fig2_migrant_distances.pdf", width = 5.5, height = 3.5)
# grid.draw(cbind(ggplotGrob(plota), ggplotGrob(plotb)))
# dev.off()
